...
describe('WorkoutScheduleService', () => {
  ...
  test('should simply adjust pattern order', async () => {
    //given
    const templateId1 = randomUUID();
    const templateId2 = randomUUID();
    const templateId3 = randomUUID();
    ...
    /*
     * test simulating popular system push pull legs with one rest day seperating workouts
     */
    const foundFinishedWorkout: Workout = {
      id: randomUUID(),
      userId: randomUUID(),
      startTime: new Date('2026-04-05T10:00:00Z'),
      usedWorkoutTemplate: templateId3,
      endTime: new Date('2026-05-10T11:00:00Z'),
      exercises: [],
    };
    const workoutSchedule: WorkoutSchedule = {
    ...
    expect(updatedWorkoutSchedule.pattern[0].useOrder).toBe(1);
    expect(updatedWorkoutSchedule.pattern[1].useOrder).toBe(2);
    expect(updatedWorkoutSchedule.pattern[2].useOrder).toBe(3);
    expect(updatedWorkoutSchedule.pattern[3].useOrder).toBe(4);
    expect(updatedWorkoutSchedule.pattern[4].useOrder).toBe(5);
    expect(updatedWorkoutSchedule.pattern[5].useOrder).toBe(0);
  });
});
